...
/*
 * 元信息类, 定义了全局的 UUID, 全局的 SchemaName, 全局的 NameSpace, 和元数据容器.
 */
public class MetaInfo
{
  ...
  // 通过全局的 UUID来构建全局的 NameSpace.
  static
  {
    GlobalNamespace.construct("Global", GlobalUUID);
  }
  // 通过全局的 NameSpace来构建元信息对象
  ...
  // 元对象信息中包括uuid, entityType, name, namespace.
  ...
  //元对象中包括name, uri, uuid, version, owner, ctime, namespace, namespaceId, EntityType和metaInfoStatus.
    ...
    implements Serializable, Cloneable
  {
    ...
    {
      return new MetaInfo.MetaObjectInfo(this);
    }
    ...
    extends MetaInfo.MetaObject
  {
    ...
    public void construct(String name, MetaInfo.Namespace ns, String className, UUID extendsType, Map<String, String> fields, List<String> keyFields, boolean generated)
    {
      ...
      this.keyFields = (List<String>) (keyFields == null ? Collections.emptyList() : keyFields);
      
      this.fields = (Map<String, String>) (fields == null ? Collections.emptyMap() : preprocessFieldNames(fields));
    ...
    extends MetaInfo.MetaObject
  {
    ...
    public void construct(String name, UUID streamUUID, MetaInfo.Namespace ns, UUID dataType, List<String> partitioningFields, GracePeriod gp, String pset)
    {
      ...
      this.partitioningFields = (List<String>) (partitioningFields == null ? Collections.emptyList() : partitioningFields);
    ...
    extends MetaInfo.MetaObject
  {
    ...
    public Set<UUID> getObjects(EntityType type)
    {
      ...
      return (Set<UUID>) (l == null ? Collections.emptySet() : l);
    }
    ...
    public Map<UUID, Set<UUID>> inEdges(Graph<UUID, Set<UUID>> graph)
    {
      ...
      for (Iterator i$ = this.objects.entrySet().iterator(); i$.hasNext();)
      {
        ...
        if ((nodes.getKey() != EntityType.TYPE) && 
          ...
          for (Iterator iterator = ((LinkedHashSet)nodes.getValue()).iterator();iterator.hasNext();) {
            try
            {
      ...
      throws Exception
    {
      ...
      if (MetaInfo.logger.isDebugEnabled())
      {
        ...
        for (Map.Entry<UUID, Set<UUID>> graphEntry : applicationGraph.entrySet())
        {
          ...
          {
        	    UUID outedgeObjectUUID = (UUID)iterator.next();
      ...
      throws Exception
    {
      ...
      for (Map.Entry<EntityType, LinkedHashSet<UUID>> nodes : this.objects.entrySet()) {
        if (nodes.getKey() == EntityType.FLOW) {
          for (Iterator iter =  ((LinkedHashSet)nodes.getValue()).iterator(); iter.hasNext();)
          {
            Flow flow = null;
    ...
    implements MetaObjectPermissionChecker
  {
    ...
    public Map<String, Object> properties;
    public com.bloom.wactionstore.Type wactionstoretype;
    public int classId = -1;
    ...
    public void construct(String name, MetaInfo.Namespace ns, UUID contextType, Interval frequency, List<UUID> eventTypes, List<String> eventKeys, Map<String, Object> properties)
    {
      ...
      this.properties = (Map<String, Object>) (properties == null ? Collections.emptyMap() : properties);
      this.wactionstoretype = com.bloom.wactionstore.Type.getType(this.properties, com.bloom.wactionstore.Type.INTERVAL);
    }
    ...
    {
      return this.wactionstoretype == com.bloom.wactionstore.Type.IN_MEMORY;
    }
    
    public boolean usesIntervalBasedWActionStore()
    {
      return this.wactionstoretype == com.bloom.wactionstore.Type.INTERVAL;
    }
    
    public boolean usesOldWActionStore()
    {
      return (this.wactionstoretype == com.bloom.wactionstore.Type.INTERVAL) || (this.wactionstoretype == com.bloom.wactionstore.Type.IN_MEMORY);
    }
    ...
    public ObjectNode getJsonForClient()
    {
      ...
      if (processedProperties.get("storageProvider") == null) {
        if (this.wactionstoretype == com.bloom.wactionstore.Type.STANDARD) {
          processedProperties.put("storageProvider", "elasticsearch");
        } else if ((this.frequency == null) && (this.wactionstoretype == com.bloom.wactionstore.Type.INTERVAL)) {
      ...
      throws Exception
    {
      ...
      String cbundleUri = WALoader.get().getBundleUri(this.nsName, BundleDefinition.Type.context, wactionContextClassName);
      ...
      throws MetaDataRepositoryException
    {
      ...
      WALoader.get().removeBundle(this.nsName, BundleDefinition.Type.context, wactionContextClassName);
      WALoader.get().removeBundle(this.nsName, BundleDefinition.Type.waction, wactionClassName);
    }
